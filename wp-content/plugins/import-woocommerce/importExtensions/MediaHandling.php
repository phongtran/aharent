	...
	public function featured_image(){
		...
		if(is_array($result)) {
			foreach($result as $val) {
				...
				$img_url = $get_image_info['value'];
				$data_array = maybe_unserialize($get_image_info['data_array']);
				$module = $get_image_info['module'];
				$image_type = $get_image_info['image_type'];
				$hash_key = $get_image_info['hash_key'];
				$image_name = basename($img_url);
				$image_title = sanitize_file_name( pathinfo( $image_name, PATHINFO_FILENAME ) );
				global $wpdb;
				if($media_handle['media_settings']['use_ExistingImage'] == false){
					global $wpdb;
					$guid_url = $img_url;
					...
					if(!empty($attachment_id)){
						...
						$attach_id = $this->image_function($img_url , $postID , $data_array ,$val->option_name);
					}
				}
				elseif($media_handle['media_settings']['overwriteImage'] == false){
					global $wpdb;
					$media_check = $wpdb->get_results("SELECT post_title FROM {$wpdb->prefix}posts WHERE post_type = 'attachment'");
					foreach($media_check as $value){
						$value_name = $value->post_title;
						if($value_name ==  $image_title){
							$get_id = $wpdb->get_results("select ID from {$wpdb->prefix}posts where post_title = '$image_title'" );
					...
					$list_of_images = $wpdb->get_results("select * from wp_ultimate_csv_importer_media where post_id = $postID ",ARRAY_A);
					if($postID == $list_of_images[0]['post_id']){
						return true;
					}
					else{
						$attach_id = $this->image_function( $img_url , $postID ,$data_array ,$val->option_name);
						if($attach_id !=null){
							$image_table = $wpdb->prefix . "ultimate_csv_importer_media";
							$medias_fields = $wpdb->get_results("INSERT INTO $image_table (image_url , attach_id , post_id,hash_key,module,image_type,status) VALUES ( '{$img_url}', $attach_id , $postID  ,'{$hash_key}','{$module}','{$image_type}','Completed')");
