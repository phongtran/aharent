...
class WC_Cancel_Order{
	...
	function clean_str($str){
		return trim(preg_replace('/ +/',' ',preg_replace('/[^A-Za-z0-9\s\.\/,\'\_\-]/',' ',urldecode(html_entity_decode(strip_tags($str))))));
	}
	...
	function add_cancel_link($order,$sent_to_admin=false,$plain_text=false){
		if(is_a($order,'WC_Order') && !$sent_to_admin && isset($this->settings['guest-cancel']) && $this->settings['guest-cancel'] && !$this->is_declined_in_past($order)){
